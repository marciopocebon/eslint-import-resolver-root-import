...
test('Supports multiple prefixes', t => {
  const prefix1 = resolve('@/file', __filename, {}, 'babel.config.array.js');
  const expected1 = expectResolvedTo('modules/file.js');

  t.deepEqual(prefix1, expected1);

  const prefix2 = resolve('_/file', __filename, {}, 'babel.config.array.js');
...
test('Can read configuration from package. when no babel.config.js found', t => {
  const actual = resolve(
    '@/file',
    __filename,
    {},
    'babel.config.nonexistent.js'
  );
  const expected = expectResolvedTo('modules/file.js');
...
test('Does nothing when babel plugin not listed in babel.config.js', t => {
  const result = resolve(
    '~/modules/file',
    __filename,
    {},
    'babel.config.notlisted.js'
  );
...
test('Support multiple file extensions', t => {
  const result1 = resolve('@/reactnative', __filename, {
    extensions: ['.android.js'],
  });

  const expected1 = expectResolvedTo('modules/reactnative/index.android.js');

  t.deepEqual(result1, expected1, 'Android');
